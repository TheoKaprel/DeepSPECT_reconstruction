...
class SPECT_system_torch(torch.nn.Module):
    def __init__(self, projections_fn, like_fn, fbprojectors,nsubsets,attmap_fn=None,geom=None):
        super().__init__()
        self.Dimension = 3
        ...
        self.geometries = self.get_geometries(geom)
    ...
    def get_geometries(self,geom):
        if geom==None:
            ...
            self.nproj = len(geometry.GetGantryAngles())
            self.nprojs_per_subsets = self.nproj // self.nsubsets
            list_sid = list(geometry.GetSourceToIsocenterDistances())
            list_angles = list(geometry.GetGantryAngles())
            geometries = []
            for subset in range(self.nsubsets):
                geometry_subset = rtk.ThreeDCircularProjectionGeometry.New()
                for i in range(self.nprojs_per_subsets):
                    geometry_subset.AddProjection(list_sid[subset + i*self.nsubsets], 0, 360/(2*np.pi)*(list_angles[subset + i*self.nsubsets]), 0,0)
                geometries.append(geometry_subset)
