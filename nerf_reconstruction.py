...
def main():
    ...
    Nx,Ny,Nz = spect.like_array.shape
    spacing = 4.7952
    print(f"Nx: {Nx}")
    print(f"Ny: {Ny}")
    print(f"Nz: {Nz}")

    x = torch.linspace(-Nx * spacing/2 + spacing/2, Nx * spacing/2 - spacing/2, Nx)
    y = torch.linspace(-Ny * spacing/2 + spacing/2, Ny * spacing/2 - spacing/2, Ny)
    z = torch.linspace(-Nz * spacing/2 + spacing/2, Nz * spacing/2 - spacing/2, Nz)
    ...
    output_ref = os.path.join(args.outputfolder, "nerf_{}iter_{}ss_{}lr_{}L_{}H_{}_{}_%d.mha".format(args.niter,
                                                                                                  args.nsubsets,
                                                                                                  args.lr,
                                                                                                  args.n_layers,
                                                                                                  args.n_features,
    ...
    if args.test:
        ...
        source_tensor = torch.Tensor(source_array).to(torch.float32).to(device)
        list_mse = []
        for iteration in range(1, args.niter + 1):
            ...
            loss = mse_loss(image_k_tensor,source_tensor)
        ...
        np.save(output_ref.replace("_%d.mha", "_lMSE.npy"), list_mse)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ref", type = str, default = "noref")
    parser.add_argument("--test", action="store_true", help="test whether or not a given architecture is capable of encoding the target image information")
